...
let slides; //concerne les slides (DOM figure)
let state; //l'état du slider à quel slide il se situe et le chrono
...
function refreshSlider()
{
    ...
    // Faire apparaître la figure demandée en lui ajoutant la classe 'active' (objet state.index)
...
function onSliderGoToNext()
{
    ...
    state.index++;
    // Est-ce qu'on est arrivé à la fin de la liste des slides ? (condition)
...
function onSliderGoToPrevious()
{
    ...
    state.index--;
    // Est-ce qu'on est revenu au début de la liste des slides ? (condition)
    if(state.index < 0) {
...
function onSliderGoToRandom()
{
    ...
    // une boucle qui renvoi des valeurs d'images au hasard (do while préféré de gilles) ici on veut éviter qu'il répète la même slide
    do {
...
function onSliderPlayPause()
{
    ...
    // Modification de l'icône du bouton play/pause ICI on doit toggle les class fa-play et fa-pause
...
document.addEventListener('DOMContentLoaded', function() {
    //à l'intérieur on gère le gestionnaire d'événement qui va se charger au moment du démarrage du site
	 ...
	 * ici on crée l'événement sur le clavier
	 */
